...
func (l *LaunchConfiguration) Find(c *fi.Context) (*LaunchConfiguration, error) {
	...
	if len(configList) == 0 {
		klog.V(2).Infof("can't found matching LaunchConfiguration: %q", *l.Name)
		return nil, nil
	}
	if len(configList) > 1 {
		klog.V(2).Infof("The number of specified ScalingConfigurations with the same name exceeds 1: %q", *l.Name)
		return nil, nil
	}
	...
	actual.ImageId = fi.String(lc.ImageId)
