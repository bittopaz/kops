...
func (c *aliyunOSSConfig) loadConfig() error {
	...
	if c.region == "" {
		...
		if err != nil {
			return fmt.Errorf("can't get region-id from ECS metadata: %v", err)
		}
	...
	if c.accessKeyId != "" {
		...
		if err != nil {
			return fmt.Errorf("Can't find role from ECS metadata: %v", err)
		}

		roleAuth, err := meta.RamRoleToken(role)
		if err != nil {
			return fmt.Errorf("Can't get role token: %v", err)
		}
